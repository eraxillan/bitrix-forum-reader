...
result_code::Type ForumThreadPool::getForumThreadPageCount(ForumThreadUrlData urlData, int &pageCount)
{
    ...
    if (m_threadPageCountCollection.contains(urlData))
    {
        ...
        ConsoleLogger->debug("Got forum thread '{}' page count ({}) from pagecount-cache", url->firstPageUrl(), pageCount);
    ...
    BFR_RETURN_VALUE_IF(result_code::failed(result),
                        result,
                        "Unable to parse first forum thread page");
    ...
    ConsoleLogger->debug("Forum thread '{}' page count ({}) was added to pagecount-cache", url->firstPageUrl(), pageCount);
...
result_code::Type ForumThreadPool::getForumPagePosts(ForumThreadUrlData urlData, int pageNo, bfr::PostList &posts)
{
    ...
    if (m_threadPagePostCollection.contains(urlData))
    {
        if (m_threadPagePostCollection.value(urlData).contains(pageNo))
        {
            ...
            ConsoleLogger->debug("Got forum thread '{}' page posts (size = {}) from pageposts-cache", url->pageUrl(pageNo), posts.size());
            return result_code::Type::Ok;
        }
    }

    // 1) Download the first forum web page
    ConsoleLogger->debug("Forum thread '{}' was not parsed yet, no page posts in the pageposts-cache", url->pageUrl(pageNo));
    ConsoleLogger->debug("Downloading first page of forum thread '{}'...", url->pageUrl(pageNo));
    QByteArray htmlRawData;
    BFR_RETURN_VALUE_IF(!FileDownloader::downloadUrl(
                            url->pageUrl(pageNo), htmlRawData,
                            std::bind(&ForumThreadPool::onDownloadProgress, this, std::placeholders::_1, std::placeholders::_2)),
                            result_code::Type::NetworkError,
                            "Unable to download specified forum thread page");
    ...
    BFR_RETURN_VALUE_IF(result_code::failed(result),
                        result,
                        "Unable to parse specified forum thread page");
    ...
    ConsoleLogger->debug("Forum thread '{}' page posts (count: {}) was added to pageposts-cache", url->pageUrl(pageNo), posts.size());
...
result_code::Type ForumThreadPool::getForumThreadPosts(ForumThreadUrlData urlData, bfr::PostList &posts)
{
    ...
    BFR_RETURN_VALUE_IF(result_code::failed(result),
                        result,
                        "Unable to get forum thread page count");
    ...
    for (int i = 1; i <= pageCount; i++)
    {
        ...
        BFR_RETURN_VALUE_IF(result_code::failed(result),
                            result,
                            "Unable to get specified forum thread page posts");
